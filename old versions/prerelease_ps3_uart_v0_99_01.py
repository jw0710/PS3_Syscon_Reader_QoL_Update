...
# --- PS3UART Klasse (Backend) - UNVERÃ„NDERT ---
...
class PS3SysconGUI:
    ...
    def _build_root_layout(self):
        ...
        customtkinter.CTkLabel(title_frame, text="Autopatch â€¢ Utility Patches â€¢ Quick & Manual Commands", font=FONT_UI, text_color="gray60").pack()
...
External Mode:
  â€¢ EEP GET <addr> <len> - Read EEPROM
  â€¢ EEP SET <addr> <len> <data> - Write EEPROM
  â€¢ ERRLOG GET 00 - Get error log (00-1F)

Internal Mode:
  â€¢ r <addr> - Read from EEPROM address
  â€¢ w <addr> <data> - Write to EEPROM address
  â€¢ errlog - Get error log
  â€¢ clearerrlog - Clear error log
  â€¢ fantbl - Fan table commands
  â€¢ patchvereep - Get patched version"""
    ...
    def _get_ps3_connection(self):
        ...
        if sc_type in ("CXR","SW"):
            serial_speed = "57600" # CXR and SW use 57600
        else:
    ...
    def _format_command_output(self, ret, sc_type):
        ...
        Diese Methode ist der SchlÃ¼ssel zur korrekten Output-Darstellung!
        """
        if ret[0] == 0xFFFFFFFF:
...
"""
    ...
    def _run_cxr_patcher(self):
        ...
        try:
            ...
            if self.sandbox_mode.get() or "FF" in response.upper():
                self._log_to_output("âœ“ Current value is FF - proceeding with patch\n")
                ...
                self._log_to_output("âœ“ Patch command sent\n")
                time.sleep(0.5)
                ...
                if self.sandbox_mode.get() or "00" in response:
                    self._log_to_output("âœ“ Patch verified successfully!\n\n")
                        ...
                        if match:
                            ...
                            if len(value_clean) == 4:
                                ...
                                if confirm:
                                    ...
                                    if ret[0] != 0xFFFFFFFF:
                                        self._log_to_output("âœ“ Correction applied!\n")
                                    ...
                                    self.output_advanced.insert("end", "âœ— Correction cancelled by user\n")
            
            if not corrections_found:
                if self.sandbox_mode.get():
                    self.output_advanced.insert("end", "\n[SANDBOX] Simulating - no real corrections needed\n")
                else:
                    self.output_advanced.insert("end", "\nâœ“ Checksum is OK - no corrections needed\n")
    ...
    def _run_rsx_checksum_correction(self):
        """RSX Swap Checksum Correction - Speziell fÃ¼r Adressen 32FE und 34FE mit Endian-Swapping"""
        ps3 = self._get_ps3_connection()
        if not ps3:
            return
        ...
        try:
            for address in rsx_addresses:
                ...
                if ret[0] == 0xFFFFFFFF:
                    self.output_advanced.insert("end", f"âœ— Failed to read address {address}: {ret[1][0]}\n")
                    continue
                ...
                # Format ist typischerweise: "00000000 XX YY" oder Ã¤hnlich
                parts = output.split()
                ...
                if len(parts) >= 3:
                    ...
                    # Frage den User ob er swappen mÃ¶chte
                    ...
                    if confirm:
                        # Schreibe die geswappten Bytes zurÃ¼ck
                        ...
                        if ret[0] != 0xFFFFFFFF:
                            self._log_to_output(f"âœ“ Swap applied successfully for 0x{address}!\n")
                            ...
                            self._log_to_output(f"âœ— Failed to write: {ret[1]}\n")
                    else:
                        self._log_to_output(f"âœ— Swap cancelled by user for 0x{address}\n")
                else:
                    self._log_to_output(f"âœ— Could not parse response for address {address}\n")
    ...
    def _execute_patch(self, commands, patch_name):
        ...
        try:
            for i, cmd in enumerate(commands, 1):
                ...
                output = self._format_command_output(ret, self.sc_type_combobox.get())
                self._log_to_output(f"âœ“ Response: {output}\n")
    ...
    def _send_manual(self, event=None):
        ...
        try:
            ...
            # DIES IST DER SCHLÃœSSEL: Verwende die korrekte Formatierung wie im alten GUI
            output = self._format_command_output(ret, self.sc_type_combobox.get())
