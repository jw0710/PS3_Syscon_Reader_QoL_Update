...
# --- DPI Awareness (verhindert Unschärfe / Pixeligkeit auf HiDPI) ---
try:
...
# --- Custom Font Loader (Windows) ---
def _load_custom_font(font_path: str) -> bool:
    """Lädt eine TTF temporär in Windows ohne Installation."""
    try:
...
def _get_heading_family() -> str:
    """
    Lädt SpaceAge.ttf und prüft via tkinter font.families() ob er wirklich
    registriert wurde. Gibt den erkannten Namen zurück, sonst Segoe UI.
    """
    base = os.path.dirname(os.path.abspath(__file__))
    font_path = os.path.join(base, "assets", "fonts", "SpaceAge.ttf")
    if not _load_custom_font(font_path):
        return "Segoe UI"
    # Tkinter kennt den Font erst nach einem Root-Fenster — daher lazy lookup
    # beim ersten Aufruf aus dem GUI-Kontext heraus.
    # Mögliche Namen die Space Age haben kann:
    for candidate in ("Space Age", "SpaceAge", "space age"):
        try:
            ...
            pass
    # Font wurde geladen aber Name noch nicht sichtbar → trotzdem probieren
    return "Space Age"


_HEADING_FAMILY = _get_heading_family()
...
FONT_HEADING = (_HEADING_FAMILY, 20, "bold")
FONT_UI = ("Segoe UI", 12)
...
class PS3SysconGUI:
    ...
    def _build_root_layout(self):
        ...
        # Left: title text
        title_text_frame = customtkinter.CTkFrame(title_frame, fg_color="transparent")
        title_text_frame.grid(row=0, column=0, sticky="w")
        customtkinter.CTkLabel(title_text_frame, text="PS3 RSX ADVANCED PATCHING", font=FONT_HEADING).pack(anchor="w")
        customtkinter.CTkLabel(title_text_frame, text="Autopatch • Utility Patches • Quick & Manual Commands", font=FONT_UI, text_color="gray60").pack(anchor="w")

        # Right: Dark/Light toggle (Apple-style)
        toggle_row = customtkinter.CTkFrame(title_frame, fg_color="transparent")
        toggle_row.grid(row=0, column=1, sticky="ne", padx=(10, 0))

        customtkinter.CTkLabel(toggle_row, text="\U0001f319", font=("Segoe UI", 14)).pack(side="left", padx=(0, 5))

        self._dark_light_var = customtkinter.BooleanVar(value=False)
        self._theme_switch = customtkinter.CTkSwitch(
            toggle_row,
            text="",
            variable=self._dark_light_var,
        ...
        customtkinter.CTkLabel(toggle_row, text="\u2600\ufe0f", font=("Segoe UI", 14)).pack(side="left", padx=(5, 0))
        ...
        # Bottom Frame: Sandbox + Helper Checkbox + Support Link
        ...
        self.helper_enabled = customtkinter.BooleanVar(value=False)
        customtkinter.CTkCheckBox(
            bottom_frame,
            text="Enable Helper (Sysko)  \u2013  may take a few seconds to load",
            variable=self.helper_enabled,
            command=self._on_helper_toggle
        ).pack(pady=(4, 0))

        self._whitemode_info = customtkinter.CTkLabel(
            bottom_frame,
            text="",
            font=("Segoe UI", 9),
            text_color="gray50"
        )
        self._whitemode_info.pack()
        ...
        # Sysko widget (initially hidden)
        self._clippy_widget = None
        self._clippy_anim_job = None
...
Internal Mode:
  ...
  Ã¢â‚¬Â¢ patchvereep - Get patched version"""
    ...
    def _get_ps3_connection(self):
        ...
        if sandbox:
            # Im Sandbox Mode keinen echten Port übergeben — verhindert ser.open()
        ...
        return PS3UART(port, sc_type, serial_speed, sandbox_mode=False)
...
"""
            ...
            if self.sandbox_mode.get() or "FF" in response.upper():
                ...
                if self.sandbox_mode.get() or "00" in response:
                    ...
                    if restart:
                        ...
                        # Restart — subprocess.Popen mit Liste verhindert
                        # Pfad-Splitting bei Leerzeichen (z.B. "Neuer Ordner")
                        import subprocess
                        script = os.path.abspath(__file__)
                        subprocess.Popen([sys.executable, script, '--cxrf-restart'])
    ...
    def _build_undervolt_tab(self):
        ...
        f.grid_rowconfigure(3, weight=0)  # content area — feste Größe
        f.grid_rowconfigure(4, weight=1)  # output log expandiert
        ...
        self.uv_content_frame.grid(row=3, column=0, padx=20, pady=10, sticky="nsew")
        ...
        output_frame.grid(row=4, column=0, padx=20, pady=(10, 20), sticky="nsew")
        ...
        self.output_undervolt.grid(row=1, column=0, sticky="nsew", padx=10, pady=(0, 8))
    ...
    def _build_auto_mode(self):
        ...
        cell_frame.grid(row=0, column=0, padx=(8, 4), pady=(8, 4), sticky="nsew")
        
        customtkinter.CTkLabel(cell_frame, text="CELL (CPU)", font=(FONT_UI[0], 13, "bold"), text_color="#cc0000").pack(pady=(8, 3))
        ...
        cell_radio_frame.pack(pady=(2, 6))
        
        customtkinter.CTkRadioButton(cell_radio_frame, text="90nm", variable=self.cell_type_var, value="90nm", font=(FONT_UI[0], 10)).pack(anchor="w", padx=20, pady=2)
        customtkinter.CTkRadioButton(cell_radio_frame, text="65nm", variable=self.cell_type_var, value="65nm", font=(FONT_UI[0], 10)).pack(anchor="w", padx=20, pady=2)
        customtkinter.CTkRadioButton(cell_radio_frame, text="45nm", variable=self.cell_type_var, value="45nm", font=(FONT_UI[0], 10)).pack(anchor="w", padx=20, pady=2)
        
        info_frame_cell = customtkinter.CTkFrame(cell_frame, fg_color="#2b2b2b", corner_radius=8)
        info_frame_cell.pack(fill="x", padx=10, pady=(2, 8))
        customtkinter.CTkLabel(info_frame_cell, text="• Lower voltage  • Reduce heat  • Extend life", font=("Consolas", 8), justify="left", text_color="gray80").pack(anchor="w", padx=8, pady=4)
        
        # === RIGHT: RSX ===
        rsx_frame = customtkinter.CTkFrame(self.auto_frame, corner_radius=10, fg_color="#1e1e1e")
        rsx_frame.grid(row=0, column=1, padx=(4, 8), pady=(8, 4), sticky="nsew")
        
        customtkinter.CTkLabel(rsx_frame, text="RSX (GPU)", font=(FONT_UI[0], 13, "bold"), text_color="#0066cc").pack(pady=(8, 3))
        ...
        rsx_radio_frame.pack(pady=(2, 6))
        
        customtkinter.CTkRadioButton(rsx_radio_frame, text="90nm", variable=self.rsx_type_var, value="90nm", font=(FONT_UI[0], 10)).pack(anchor="w", padx=20, pady=2)
        customtkinter.CTkRadioButton(rsx_radio_frame, text="65nm", variable=self.rsx_type_var, value="65nm", font=(FONT_UI[0], 10)).pack(anchor="w", padx=20, pady=2)
        customtkinter.CTkRadioButton(rsx_radio_frame, text="40nm", variable=self.rsx_type_var, value="40nm", font=(FONT_UI[0], 10)).pack(anchor="w", padx=20, pady=2)
        
        info_frame_rsx = customtkinter.CTkFrame(rsx_frame, fg_color="#2b2b2b", corner_radius=8)
        info_frame_rsx.pack(fill="x", padx=10, pady=(2, 8))
        customtkinter.CTkLabel(info_frame_rsx, text="• Chip-specific  • Lower GPU temp  • Test w/ games", font=("Consolas", 8), justify="left", text_color="gray80").pack(anchor="w", padx=8, pady=4)
        
        # Buttons
        self.cell_uv_button_auto = customtkinter.CTkButton(self.auto_frame, text="Apply CELL", command=self._apply_cell_undervolt, font=(FONT_UI[0], 12, "bold"), height=40, fg_color="#cc0000", hover_color="#990000")
        self.cell_uv_button_auto.grid(row=1, column=0, padx=(8, 4), pady=(4, 4), sticky="ew")
        
        self.rsx_uv_button_auto = customtkinter.CTkButton(self.auto_frame, text="Apply RSX", command=self._apply_rsx_undervolt, font=(FONT_UI[0], 12, "bold"), height=40, fg_color="#0066cc", hover_color="#004499")
        self.rsx_uv_button_auto.grid(row=1, column=1, padx=(4, 8), pady=(4, 4), sticky="ew")
        
        # Reverse checkbox
        reverse_frame = customtkinter.CTkFrame(self.auto_frame, fg_color="transparent")
        reverse_frame.grid(row=2, column=0, columnspan=2, padx=10, pady=(0, 6))
        ...
        self.reverse_checkbox.pack(pady=3)
    ...
    def _build_manual_mode(self):
        ...
        cell_slider_frame.pack(fill="x", padx=12, pady=(10, 5))
        
        customtkinter.CTkLabel(cell_slider_frame, text="CELL (CPU) Voltage", font=(FONT_UI[0], 12, "bold"), text_color="#cc0000").pack(pady=(8, 4))
        ...
        self.cell_voltage_label.pack(pady=(0, 8))
        
        # RSX Slider
        rsx_slider_frame = customtkinter.CTkFrame(self.manual_frame, corner_radius=10, fg_color="#1e1e1e")
        rsx_slider_frame.pack(fill="x", padx=12, pady=5)
        
        customtkinter.CTkLabel(rsx_slider_frame, text="RSX (GPU) Voltage", font=(FONT_UI[0], 12, "bold"), text_color="#0066cc").pack(pady=(8, 4))
        ...
        self.rsx_voltage_label.pack(pady=(0, 8))
        ...
        self.cell_uv_button_manual = customtkinter.CTkButton(btn_frame, text="Apply CELL", command=self._apply_cell_manual, font=(FONT_UI[0], 12, "bold"), height=40, fg_color="#cc0000", hover_color="#990000")
        self.cell_uv_button_manual.grid(row=0, column=0, padx=5, sticky="ew")
        
        self.rsx_uv_button_manual = customtkinter.CTkButton(btn_frame, text="Apply RSX", command=self._apply_rsx_manual, font=(FONT_UI[0], 12, "bold"), height=40, fg_color="#0066cc", hover_color="#004499")
    ...
    def _on_theme_toggle(self):
        """Dark/Light toggle oben rechts – unabhängig von Sysko."""
        if self._dark_light_var.get():
            customtkinter.set_appearance_mode("light")
        else:
            customtkinter.set_appearance_mode("dark")
            # Sysko braucht Light Mode – beim Abdunkeln also auch Sysko deaktivieren
            if self.helper_enabled.get():
                self.helper_enabled.set(False)
                self._clippy_hide()
                self._whitemode_info.configure(text="")

    def _on_helper_toggle(self):
        """Sysko-Haken – aktiviert automatisch Light Mode."""
        if self.helper_enabled.get():
            # Light Mode einschalten (Toggle-Switch mitziehen)
            self._dark_light_var.set(True)
            customtkinter.set_appearance_mode("light")
            self._whitemode_info.configure(text="Light Mode  \u2022  Sysko is watching you")
            self._clippy_show()
        else:
            customtkinter.set_appearance_mode("dark")
            self._dark_light_var.set(False)
            self._whitemode_info.configure(text="")
            self._clippy_hide()

    def _clippy_show(self):
        """Platziert Sysko unten rechts im Hauptfenster (chippy.gif)."""
        if self._clippy_widget is not None:
            return

        assets   = self._get_assets_dir()
        gif_path = os.path.join(assets, "chippy.gif")
        SIZE     = (108, 108)
        # Hintergrundfarbe je nach Mode
        light = customtkinter.get_appearance_mode() == "Light"
        BG       = "#ebebeb" if light else "#222220"
        BG_TUPLE = (235, 235, 235, 255) if light else (34, 34, 32, 255)
        self._clippy_bg_tuple = BG_TUPLE

        self._clippy_static_frame = None
        self._clippy_anim_frames  = []
        self._clippy_anim_idx     = 0

        try:
            gif = Image.open(gif_path)
            # Ersten Frame sofort laden → Chippy erscheint direkt
            first_rgba = gif.copy().convert("RGBA").resize(SIZE, Image.LANCZOS)
            bg = Image.new("RGBA", SIZE, BG_TUPLE)
            bg.paste(first_rgba, mask=first_rgba)
            self._clippy_static_frame = ImageTk.PhotoImage(bg.convert("RGB"))
            # Alle restlichen Frames lazy über after() nachladen
            self._clippy_gif_obj     = gif
            self._clippy_gif_size    = SIZE
            self._clippy_gif_loading = True
        except Exception:
            self._clippy_gif_loading = False

        if self._clippy_static_frame is None:
            return

        lbl = tk.Label(
            self.root,
            image=self._clippy_static_frame,
            bg=BG,
            cursor="hand2",
            bd=0,
            highlightthickness=0
        )
        # Weiter in die Ecke: 4px Abstand
        lbl.place(relx=1.0, rely=1.0, anchor="se", x=-4, y=-4)

        lbl.bind("<Enter>", self._clippy_hover_start)
        lbl.bind("<Leave>", self._clippy_hover_stop)
        lbl.bind("<Button-1>", self._clippy_open_menu)

        self._clippy_widget = lbl

        # Restliche Frames im Hintergrund laden (nach 100ms starten)
        self.root.after(100, self._clippy_load_remaining_frames)

    def _clippy_load_remaining_frames(self):
        """Lädt alle GIF-Frames schrittweise nach – je 10 Frames pro Durchlauf."""
        if not getattr(self, "_clippy_gif_loading", False):
            return
        gif  = self._clippy_gif_obj
        size = self._clippy_gif_size
        # Aktuell geladene Anzahl = len(anim_frames); 0 = noch keiner
        batch = 0
        try:
            # Seekposition: wir starten bei frame index len(anim_frames)+1
            # (Frame 0 ist bereits als static geladen)
            target_start = len(self._clippy_anim_frames) + 1
            gif.seek(target_start)
            while batch < 10:
                frame_rgba = gif.copy().convert("RGBA").resize(size, Image.LANCZOS)
                bg = Image.new("RGBA", size, getattr(self, "_clippy_bg_tuple", (34, 34, 32, 255)))
                bg.paste(frame_rgba, mask=frame_rgba)
                self._clippy_anim_frames.append(ImageTk.PhotoImage(bg.convert("RGB")))
                gif.seek(gif.tell() + 1)
                batch += 1
            # Noch mehr zu laden → nächsten Batch planen
            self.root.after(10, self._clippy_load_remaining_frames)
        except EOFError:
            # Alle Frames geladen
            self._clippy_gif_loading = False

    def _clippy_hide(self):
        """Entfernt Chippy vom Fenster."""
        if self._clippy_anim_job is not None:
            self.root.after_cancel(self._clippy_anim_job)
            self._clippy_anim_job = None
        if self._clippy_widget is not None:
            self._clippy_widget.destroy()
            self._clippy_widget = None

    def _clippy_hover_start(self, event):
        self._clippy_anim_idx = 0
        self._clippy_animate()

    def _clippy_hover_stop(self, event):
        if self._clippy_anim_job is not None:
            self.root.after_cancel(self._clippy_anim_job)
            self._clippy_anim_job = None
        if self._clippy_widget and self._clippy_static_frame:
            self._clippy_widget.configure(image=self._clippy_static_frame)

    def _clippy_animate(self):
        if not self._clippy_widget or not self._clippy_anim_frames:
            return
        self._clippy_widget.configure(image=self._clippy_anim_frames[self._clippy_anim_idx])
        self._clippy_anim_idx = (self._clippy_anim_idx + 1) % len(self._clippy_anim_frames)
        self._clippy_anim_job = self.root.after(80, self._clippy_animate)

    def _clippy_open_menu(self, event):
        menu = tk.Menu(self.root, tearoff=0,
                       bg="#1e1e2e", fg="white",
                       activebackground="#3a3a6a", activeforeground="white",
                       font=("Segoe UI", 11),
                       bd=0, relief="flat")
        menu.add_command(label="Serial Connection Points", command=self._clippy_serial_points)
        menu.add_command(label="Errorcodes",               command=self._clippy_errorcodes)
        menu.add_command(label="Setup",                    command=self._clippy_setup)
        try:
            menu.tk_popup(event.x_root, event.y_root - menu.winfo_reqheight())
        finally:
            menu.grab_release()

    # ── Serial Connection Points ──────────────────

    def _clippy_serial_points(self):
        models = [
            ("All Super Slim Models",
             [("SW3_SC_UART_testpads", "All-Super-Slim-Models/SW3_SC_UART_testpads.jpg")]),
            ("COK-001/002 (A-E Models)",
             [("COK-001_SC_UART_testpads", "COK-001-002-(A-E-Models)/COK-001_SC_UART_testpads.jpg")]),
            ("DIA-001/002 (H-K Models)",
             [("DIA-001_SC_UART_testpads", "DIA-001-002-(H-K-Models)/DIA-001_SC_UART_testpads.jpg"),
              ("DIA-002_SC_UART_testpads", "DIA-001-002-(H-K-Models)/DIA-002_SC_UART_testpads.jpg")]),
            ("DYN-001 (20xx Model)",
             [("DYN-001_SC_UART_testpads", "DYN-001-(20xx-Model)/DYN-001_SC_UART_testpads.jpg")]),
            ("SEM-001 (G Models)",
             [("SEM-001_SC_UART_testpads", "SEM-001-(G-Models)/SEM-001_SC_UART_testpads.jpg")]),
            ("SUR-001/JTP-001/JSD-001/KTE-001 (21xx-40xx Models)",
             [("SUR-001_SC_and_SB_UART_testpads", "SUR-001-JTP-001-JSD-001-KTE-001-(21xx-40xx-Models)/SUR-001_SC_and_SB_UART_testpads.jpg")]),
            ("VER-001 (L-Q Models)",
             [("VER-001_SC_UART_testpads", "VER-001-(L-Q-Models)/VER-001_SC_UART_testpads.jpg")]),
        ]

        win = tk.Toplevel(self.root)
        win.title("Sysko – Serial Connection Points")
        win.configure(bg="#1a1a2e")
        win.resizable(False, False)

        tk.Label(win, text="Select your PS3 Model:", font=("Segoe UI", 13, "bold"),
                 bg="#1a1a2e", fg="#ffe066").pack(pady=(14, 4))

        assets_base = os.path.join(self._get_assets_dir(), "PS3-Serial-Connection-Points")

        for model_name, images in models:
            grp = tk.LabelFrame(win, text=model_name, bg="#1a1a2e", fg="#aaaadd",
                                font=("Segoe UI", 10, "bold"), bd=1, relief="groove")
            grp.pack(fill="x", padx=18, pady=3)
            for img_label, img_rel_path in images:
                full_path = os.path.join(assets_base, img_rel_path.replace("/", os.sep))
                tk.Button(
                    grp,
                    text=f"  {img_label}",
                    bg="#1a3a5c", fg="white",
                    activebackground="#2563ae", activeforeground="white",
                    font=("Segoe UI", 10), relief="flat", cursor="hand2",
                    anchor="w",
                    command=lambda p=full_path, t=img_label: self._show_image_viewer(p, t)
                ).pack(fill="x", padx=8, pady=2)

        tk.Button(win, text="Close", bg="#3a3a5c", fg="#aaaacc",
                  font=("Segoe UI", 10), relief="flat", cursor="hand2",
                  command=win.destroy, pady=4).pack(pady=(10, 14), padx=18, fill="x")

        self._center_window(win)

    def _show_image_viewer(self, path, title):
        win = tk.Toplevel(self.root)
        win.title(f"Sysko – {title}")
        ...
        try:
            ...
            lbl = tk.Label(win, image=photo, bg="#0d0d1a")
            lbl.image = photo
            lbl.pack(padx=10, pady=10)
        except Exception as e:
            tk.Label(win, text=f"Could not load image:\n{e}",
                     bg="#0d0d1a", fg="#ff4444", font=("Segoe UI", 11)).pack(padx=20, pady=20)

        tk.Button(win, text="Close", bg="#3a3a5c", fg="#aaaacc",
                  font=("Segoe UI", 10), relief="flat", cursor="hand2",
                  command=win.destroy, pady=4).pack(pady=(0, 10), padx=20, fill="x")

        self._center_window(win)

    # ── Errorcodes ────────────────────────────────

    def _clippy_errorcodes(self):
        """Öffnet die Error-Codes-HTML mit dem Standardprogramm des Systems."""
        html_path = os.path.join(self._get_assets_dir(), "Errors",
                                "Error Codes - PS3 Developer wiki.html")
        try:
            os.startfile(html_path)
        except Exception as e:
            messagebox.showerror("Sysko", f"Could not open HTML:\n{e}")

    # ── Setup Guide ───────────────────────────────

    def _clippy_setup(self):
        steps = [
            ("1. Hardware", [
                "Connect a USB-to-UART adapter (CP2102, CH340, FT232, etc.) to your PC.",
                "Wire TX→RX, RX→TX, GND→GND to the PS3 Syscon UART testpads.",
                "Use 'Serial Connection Points' to find the exact pads for your model.",
            ]),
            ("2. Drivers", [
                "Install the driver for your adapter.",
                "CP2102: Silicon Labs  |  CH340: WCH  |  FT232: FTDI",
                "After install, the COM port appears in Device Manager.",
            ]),
            ("3. Software", [
                "Install Python 3.10+ (add to PATH).",
                "pip install pyserial pycryptodomex customtkinter pillow",
                "Run NostaDiag_PS3_v1.00.py",
            ]),
            ("4. Connection", [
                "Select the correct COM port in the 'Serial Port' dropdown.",
                "SC Type:  CXR (fat/slim)  |  CXRF (Mullion patched)  |  SW (Super Slim)",
                "Click 'Refresh' if the port is not listed.",
            ]),
            ("5. Authenticate", [
                "Go to 'Advanced Patching' and click 'AUTH PS3' before any patching.",
                "Green status = authentication successful.",
                "Without AUTH, RSX patching is locked to prevent bricking.",
            ]),
            ("6. Sandbox Mode", [
                "Tick 'Sandbox Mode' to simulate without real hardware.",
                "Commands are echoed to output but no UART traffic is sent.",
            ]),
        ]

        win = tk.Toplevel(self.root)
        win.title("Sysko – Setup Guide")
        win.configure(bg="#1a1a2e")
        win.resizable(True, True)

        tk.Label(win, text="Setup Guide – Getting Started",
                 font=("Segoe UI", 13, "bold"), bg="#1a1a2e", fg="#ffe066").pack(pady=(14, 6))

        container = tk.Frame(win, bg="#1a1a2e")
        container.pack(fill="both", expand=True, padx=18, pady=4)

        canvas = tk.Canvas(container, bg="#1a1a2e", highlightthickness=0, width=560, height=400)
        ...
        for section, bullets in steps:
            sf = tk.LabelFrame(inner, text=section, bg="#1a1a2e", fg="#ffe066",
                               font=("Segoe UI", 10, "bold"), bd=1, relief="groove")
            sf.pack(fill="x", pady=4, padx=2)
            for b in bullets:
                tk.Label(sf, text=f"  •  {b}", bg="#1a1a2e", fg="#ccccee",
                         font=("Segoe UI", 9), anchor="w", justify="left",
                         wraplength=500).pack(fill="x", padx=8, pady=1)
        ...
        tk.Button(win, text="Close", bg="#3a3a5c", fg="#aaaacc",
                  font=("Segoe UI", 10), relief="flat", cursor="hand2",
                  command=win.destroy, pady=4).pack(pady=(8, 14), padx=18, fill="x")
...
def main():
    ...
    root.geometry("1140x980")

    # Font-Familie recheck
    global FONT_HEADING
    import tkinter.font as tkfont
    families = tkfont.families()
    for candidate in ("Space Age", "SpaceAge", "space age"):
        if candidate in families:
            FONT_HEADING = (candidate, 20, "bold")
            break
